...
for row = 1:N
    for col = 1:N
        if P.edges(row, col) ... clique(row) connected to clique(col)
           && isempty(messages(row, col).var) % no message from clique(row) to clique(col)
            inputs = ones(N, 1); 
            for idx = 1:N
                if P.edges(idx, row) % check the incoming messages from neighbors
                    inputs(idx) = ~isempty(messages(idx, row).var);
                end
            end
            inputs(col) = 1; % the potential receiver's message is not required
            if all(inputs) % all neighbors except the receiver have sent the message
                i = row;
                j = col;
                return;
            end
