...
for idx = 1:(2 * numAlleles * numAlleles)
    assignment = IndexToAssignment(idx, phenotypeFactor.card);
    phenotype = assignment(1);
    alleleCopyVarOne = assignment(2);
    alleleCopyVarTwo = assignment(3);
    genotype = allelesToGenotypes(alleleCopyVarOne, alleleCopyVarTwo);
    if (phenotype == 1)
        phenotypeFactor.val(idx) = alphaList(genotype);
    else
        phenotypeFactor.val(idx) = 1 - alphaList(genotype);
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%