...
for idx = 1:numPeople
    ...
    if (parents(1) == 0) % root in the family tree
        factorList(idx) = childCopyGivenFreqsFactor(alleleFreqs, idx);
    else % has parents in the family tree
        factorList(idx) =  childCopyGivenParentalsFactor(numAlleles, idx, parents(1), parents(1) + numPeople);
    end
end
for idx = (numPeople + 1):(2 * numPeople)
    parents = pedigree.parents(idx - numPeople, :);
    if (parents(1) == 0) % root in the family tree
        factorList(idx) = childCopyGivenFreqsFactor(alleleFreqs, idx);
    else % has parents in the family tree
        factorList(idx) =  childCopyGivenParentalsFactor(numAlleles, idx, parents(2), parents(2) + numPeople);
    end
end
for idx = (2 * numPeople + 1):(3 * numPeople)
    factorList(idx) = ... 
        phenotypeGivenCopiesFactor(alphaList, numAlleles, idx - 2 * numPeople, idx - numPeople, idx);
end
