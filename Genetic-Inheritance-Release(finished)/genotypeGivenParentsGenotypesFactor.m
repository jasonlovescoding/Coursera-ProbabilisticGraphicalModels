...
m = numAlleles * (numAlleles + 1) / 2; % number of possible geno-combinations
genotypeFactor.card = [m, m, m];
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
idxNum = m^3; % number of child-parents combinations
for idx = 1:idxNum
    assignment = IndexToAssignment(idx, genotypeFactor.card);
    allelesVarChild = genotypesToAlleles(assignment(1), :);
    allelesVarParentOne = genotypesToAlleles(assignment(2), :);
    allelesVarParentTwo = genotypesToAlleles(assignment(3), :);
    % all possible child genotypes in columns
    allelesVarChildPossible = combvec(allelesVarParentOne, allelesVarParentTwo); 
    % find the number of occurence of the child's genotype
    allelesHitCount = sum(all(bsxfun(@eq, allelesVarChild', allelesVarChildPossible), 1)); 
    % if the child's genotype is heterozygous, flip over and count the other case 
    if (allelesVarChild(1) ~= allelesVarChild(2))
        allelesHitCount = allelesHitCount + ...
            sum(all(bsxfun(@eq, flipud(allelesVarChild'), allelesVarChildPossible), 1));
    end
    genotypeFactor.val(idx) = allelesHitCount / size(allelesVarChildPossible, 2);             
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%