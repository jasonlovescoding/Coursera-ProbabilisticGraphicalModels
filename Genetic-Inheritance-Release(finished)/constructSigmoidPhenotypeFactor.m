...
m = size(alleleWeights, 2); % number of genes involved
for j = 1:m
    n = length(alleleWeights{j}); % number of alleles of gene m
    phenotypeFactor.card = cat(2, phenotypeFactor.card, [n, n]);
end

phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
for idx = 1:length(phenotypeFactor.val)
    assignment = IndexToAssignment(idx, phenotypeFactor.card);
    phenotype = assignment(1);
    weight = 0;
    for j = 1:m
        alleleCopyVarOne = assignment(j + 1);
        alleleCopyVarTwo = assignment(j + 1 + m);
        weight = weight + alleleWeights{j}(alleleCopyVarOne) + alleleWeights{j}(alleleCopyVarTwo);
    end
    if (phenotype == 1)
        phenotypeFactor.val(idx) = computeSigmoid(weight);
    else
        phenotypeFactor.val(idx) = 1 - computeSigmoid(weight);
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%